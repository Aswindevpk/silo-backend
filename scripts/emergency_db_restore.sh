...
LATEST_DUMP=$(docker compose exec -T -e AWS_ACCESS_KEY_ID="$AWS_ACCESS_KEY_ID" -e AWS_SECRET_ACCESS_KEY="$AWS_SECRET_ACCESS_KEY" backup-sidecar sh -c "aws s3 ls s3://${BACKUP_S3_BUCKET}/backup/ --endpoint-url ${AWS_S3_ENDPOINT_URL} --region auto | sort | tail -n 1 | awk '{print \$4}'")
echo "   Found latest backup: $LATEST_DUMP"
docker compose exec -T -e AWS_ACCESS_KEY_ID="$AWS_ACCESS_KEY_ID" -e AWS_SECRET_ACCESS_KEY="$AWS_SECRET_ACCESS_KEY" backup-sidecar sh -c "aws s3 cp s3://${BACKUP_S3_BUCKET}/backup/$LATEST_DUMP /tmp/latest_backup.dump --endpoint-url ${AWS_S3_ENDPOINT_URL} --region auto"
...
docker compose exec -T db psql -U "$DB_USER" -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$DB_NAME';" || true
...
docker compose start web celery || true
